...
// Directory in the repo -> the skill directory name it installs as. Claude Code
// discovers a skill by its directory name, and the repo's directories are named
// for the surface (`mcp`, `cli`) rather than the skill (`pingroom-mcp`), so the
// rename is part of a correct install, not a cosmetic choice.
const SKILLS = [
  { source: 'mcp', install: 'pingroom-mcp', summary: 'conversational agents — the hosted MCP connector' },
  { source: 'cli', install: 'pingroom-cli', summary: 'shells, CI, and Claude Code hooks' },
];
...
${SKILLS.map((s) => `  cp -r /tmp/pingroom-skills/${s.source} ${join(claudeSkillsDir(), s.install)}`).join('\n')}
  ...
  try {
    for (const skill of SKILLS) {
      ...
      if (!directoryExists(from)) {
        fail(`${SKILLS_REPO} has no "${skill.source}" directory — the repo layout changed.`, EXIT.ERROR);
      }
